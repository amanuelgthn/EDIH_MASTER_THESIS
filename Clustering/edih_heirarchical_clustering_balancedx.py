#!/usr/bin/env python3
"""
Country-level SST Specialization — Hierarchical Clustering (k = 2..6) with Multi-Metric Model Selection

- Input: 'combined_further_cleaned_keywords (1).xlsx' (Sheet 1)
- Canonical columns mapped from:
    "Formatted sectors"       -> "Sector"
    "Formatted services"      -> "Service"
    "Formatted technologies"  -> "Technology"
  Country auto-detected (see COUNTRY_HINTS).

- Evaluates for each k and method (Ward/Euclid, Average/Cosine):
    * Silhouette (higher better)
    * Calinski-Harabasz (CH; higher better)
    * Davies-Bouldin (DB; lower better -> inverted when scoring)
    * Size balance via normalized entropy (0..1; higher = more even)
    * Tiny-cluster penalty (clusters below MIN_CLUSTER_SHARE)

- Selects best (k, method) via composite score.
- Also exports forced solutions for k in {3,4,5,6}.

Outputs:
    Excel: Country_clusters_hier_k2_k6_SELECTION.xlsx
...
SHEET_NAME     = 0
OUTPUT_FILE    = "Country_clusters_hier_k2_k6_SELECTION.xlsx"
PLOT_DIR       = "country_cluster_plots_balanced"
...
# Scoring
BALANCE_WEIGHT     = 0.5   # weight for entropy in composite
SIL_WEIGHT         = 0.35  # weight for silhouette
CH_WEIGHT          = 0.10  # weight for Calinski-Harabasz (normalized)
DB_WEIGHT          = 0.05  # weight for (inverted) Davies-Bouldin (normalized)
MIN_CLUSTER_SHARE  = 0.05
PENALTY_PER_TINY   = 0.2
...
# Normalize metrics to [0,1] across candidates at the same k
def minmax(x):
...
def country_summary(df_in):
    ...
    for country, g in df_in.groupby("Country"):
        ...
        rows.append({
            ...
            "X5_sector_top5_share": sec_sh.head(5).sum() if not sec_sh.empty else np.nan,
            "X6_tech_top5_share": tec_sh.head(5).sum() if not tec_sh.empty else np.nan,
            "X7_service_top5_share": srv_sh.head(5).sum() if not srv_sh.empty else np.nan
        })
...
for k in K_SCAN:
    ...
    db_w  = davies_bouldin_score(Xc_emb, lab_w)  # lower better

    candidates[(k, "Ward")] = dict(labels=lab_w, sizes=sizes_w, props=props_w,
                                   Hn=Hn_w, sil=sil_w, ch=ch_w, db=db_w)
    ...
    candidates[(k, "CosineAvg")] = dict(labels=lab_c, sizes=sizes_c, props=props_c,
                                        Hn=Hn_c, sil=sil_c, ch=ch_c, db=db_c)
...
for k in K_SCAN:
    ...
    db_n  = minmax(db_inv)  # higher is better now
...
# =========================
def build_top_for(key_k_method):
    k, method = key_k_method
    res = chosen[(k,method)]
    labels = res["labels"]
...
with pd.ExcelWriter(OUTPUT_FILE, engine="openpyxl") as writer:
    ...
    _sc = _SS()
    z = pd.DataFrame(_sc.fit_transform(summary_tbl),
                     index=summary_tbl.index, columns=[f"Z_{c}" for c in summary_tbl.columns]).reset_index().rename(columns={"index":"Country"})
    ...
    # Forced outputs for k = 3,4,5,6 (both methods) so you can compare
